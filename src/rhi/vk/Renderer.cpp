...
namespace maj::rhi
{
	...
	Renderer::SingleTimeCommand::~SingleTimeCommand()
	{
		...
		VkResult result { vkQueueSubmit(kDevice->queueHandler().queue("graphics"), 1u, &submitInfo, VK_NULL_HANDLE) };
		if (result != VK_SUCCESS)
			LOG(LogRHIWarning, "failed to submit single time command: " << result)
