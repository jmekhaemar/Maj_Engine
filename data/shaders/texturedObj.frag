...
vec3 lightUpdate(uint index, vec3 N, vec3 V)
{
	...
	finalColor += lights.light[index].color * clamp(pow(max(dot(N, H), 0.0), 32) * lights.light[index].values.z * intensity, 0.0, 1.0);
	return finalColor;
}
...
void main() 
{
	...
	outColor = texture(texSampler, fragUV).rgb * finalColor;
}